...
var b2 = { regex: { rules: { class: { pattern: /^\.([a-zA-Z][\w-]*)$/, resolve(e) {
  ...
  let a2 = function(l, o = {}, c = []) {
    if (c.includes(this.path))
      return console.warn(`Circular reference detected at ${this.path}`), { colorScheme: l, path: this.path, paths: o, value: void 0 };
    ...
    Object.entries(l).forEach(([m2, d]) => {
      ...
      i(d) ? n(d, u, h) : (i2[u] || (i2[u] = { paths: [], computed: (f, x = {}, p2 = []) => {
        if (i2[u].paths.length === 1)
          return i2[u].paths[0].computed(i2[u].paths[0].scheme, x.binding, p2);
        if (f && f !== "none")
          for (let y = 0; y < i2[u].paths.length; y++) {
            let R = i2[u].paths[y];
            if (R.scheme === f)
              return R.computed(f, x.binding, p2);
          }
...
//# sourceMappingURL=chunk-QDFQOKUI.js.map
